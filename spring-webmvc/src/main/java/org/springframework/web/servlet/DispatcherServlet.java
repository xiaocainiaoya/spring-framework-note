...
public class DispatcherServlet extends FrameworkServlet {
	...
	static {
		...
		try {
			// 从DispatcherServlet.properties文件加载默认的组件生成策略(键值对 --> 键为全限定类名, 值为具体的实现类)
	...
	/**
	 * 获取springBean容器中的multipartResolver
	 *
	...
	/**
	 * 初始化此类使用的 HandlerMappings。如果在 BeanFactory 中没有为这个命名空间定义 HandlerMapping bean，我们默认为 BeanNameUrlHandlerMapping。
	 *
	...
	private void initHandlerMappings(ApplicationContext context) {
		...
		// 检测所有 HandlerMapping 还是只期望“handlerMapping”bean？。
		...
		// 默认的是：BeanNameUrlHandlerMapping、RequestMappingHandlerMapping、RouterFunctionMapping
	...
	private void initHandlerAdapters(ApplicationContext context) {
		...
		// 检测所有 HandlerAdapter 还是只期望“handlerAdapter”bean？。
	...
	/**
	 * 它是一个范型的方法，承担所有SpringMVC编程元素的默认初始化策略。方法的内容比较直白，就是以传递类的名称为键，
	 * 从defaultStrategies这个Properties变量中获取实现类，然后反射初始化。
	 *
	...
	protected void doDispatch(HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		try {
			...
			try {
				...
				// 根据HttpServletRequest获取匹配的Handler
				...
				// 根据handler获取第一个匹配的HandlerAdapter
